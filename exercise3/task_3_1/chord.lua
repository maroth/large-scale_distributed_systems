...
function query()
    for i = 1, number_of_queries do
        ...
        if is_in_range(key_to_find, self_node, fingers[1]) then
            ...
            if use_fingers then
                -- if we can use fingers, we find the node preceeding the key we want to find and send the query there
                local hops = 0
                local cursor = self_node
                local cursor_successor = fingers[1]
                while not is_in_range_numeric(tonumber(key_to_find), normalize_id(get_id(cursor)), normalize_id(get_id(cursor_successor)), false, true) do
                    cursor = rpc.call(cursor, {"closest_preceeding_finger", key_to_find})
                    hops = hops + 1
                    cursor_successor = rpc.call(cursor, {"get_successor"})
                end
                logS("key_found " .. hops)
            else
                -- if we can't use fingers, we simply follow the ring successor by successor
...
function find_predecessor(id)
    ...
    debug_print("find_predecessor done, returning " .. normalize_id(get_id(cursor)))
    return cursor
end
...
function init_finger_table(anchor_node)
    debug_print("\n\ninit_finger_table of new node " .. get_own_key() .. " with anchor node " .. normalize_id(get_id(anchor_node)))
...
function update_finger_table(target_node, finger_number) 
    ...
    if current_finger_is_perfect then debug_print("the current finger is already perfect") else debug_print("the current finger is not perfect") end
    ...
    if new_finger_is_better then debug_print("the new finger is better") else debug_print("the new finger is not better") end
...
function run_tests()
    -- always print debug messages in test modefalse
    print_debug_messages = true
...
function test_get_finger_source()
    ...
    assert(get_finger_source(1) == 24, "finger 1 source " .. get_finger_source(1))
    assert(get_finger_source(2) == 23, "finger 2 source " .. get_finger_source(2))
    assert(get_finger_source(3) == 21, "finger 3 source " .. get_finger_source(3))
    assert(get_finger_source(4) == 17, "finger 4 source " .. get_finger_source(4))
    assert(get_finger_source(5) == 9, "finger 5 source " .. get_finger_source(5))
    assert(get_finger_source(6) == 249, "finger 6 source " .. get_finger_source(6))
    assert(get_finger_source(7) == 217, "finger 7 source " .. get_finger_source(7))
    assert(get_finger_source(8) == 153, "finger 8 source " .. get_finger_source(8))
end
